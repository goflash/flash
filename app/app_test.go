...
func TestAppHealthCheck(t *testing.T) {
	...
	if response["service"] != "goflash" {
		t.Fatalf("expected service 'goflash', got %v", response["service"])
	}
...
func TestAppHealthCheckWithCustomFunction(t *testing.T) {
	app := New()

	// Test with custom health check that passes
	app.EnableHealthCheck("/health")
...
func TestAppHealthCheckWithCustomFunctionFailure(t *testing.T) {
	app := New()

	// Test with custom health check that fails
	app.EnableHealthCheck("/health")
	app.SetHealthCheck(func() error {
...
func TestAppHealthCheckPath(t *testing.T) {
	app := New()

	// Test that health check path is set correctly
	path := "/health"
	app.EnableHealthCheck(path)

	if app.HealthCheckPath() != path {
		t.Fatalf("expected health check path %s, got %s", path, app.HealthCheckPath())
	}
}

func TestAppHealthCheckMultiplePaths(t *testing.T) {
	app := New()

	// Test that we can change health check path
	app.EnableHealthCheck("/health")
	app.EnableHealthCheck("/status")

	if app.HealthCheckPath() != "/status" {
		t.Fatalf("expected health check path /status, got %s", app.HealthCheckPath())
	}

	// Test that both paths work (httprouter behavior)
	req := httptest.NewRequest(http.MethodGet, "/health", nil)
	rec := httptest.NewRecorder()
	app.ServeHTTP(rec, req)

	if rec.Code != http.StatusOK {
		t.Fatalf("expected status 200 for old path, got %d", rec.Code)
	}
	...
	if rec.Code != http.StatusOK {
		t.Fatalf("expected status 200 for new path, got %d", rec.Code)
	}
}
